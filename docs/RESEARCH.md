...
| **Blockchain** | Base Sepolia (L2) | Phí gas cực rẻ, kế thừa bảo mật từ Ethereum, ecosystem Base phát triển mạnh |

### 1.2 Tại sao dùng Base Sepolia (L2) thay vì Ethereum L1?

- **Tối ưu chi phí:** Phí gas trên L2 cực rẻ (chỉ một phần nhỏ so với L1), phù hợp cho marketplace có nhiều giao dịch (deposit, release, refund).
- **Kế thừa bảo mật:** Base vẫn kế thừa tính bảo mật từ Ethereum L1.
- **Ecosystem:** Được chống lưng bởi Coinbase, Base ecosystem đang bùng nổ.
- **Testnet Base Sepolia:** Nhanh, faucet dễ lấy, đầy đủ tool (Basescan, Alchemy, Hardhat).
...
| **L1 / L2** | L1 = blockchain gốc (Ethereum). L2 = mạng "xây trên" L1 (Arbitrum, Base), kế thừa bảo mật nhưng rẻ + nhanh hơn. Bloody-Roar chạy trên **L2** (Base Sepolia) — rẻ + nhanh. |
...
|-----------|-------|---------|------------|----------|
| **EAS** (Ethereum Attestation Service) | Open-source primitive cho phép tạo "lời xác nhận" on-chain/off-chain | Free, chuẩn chung (interoperable), revocable, native trên Ethereum L1 (hỗ trợ Base) | Cần học khái niệm attestation | ✅ **Nên dùng cho reputation/skill** |
